...
void push_scope(struct parser *p)
{
	...
	p->num_stk++;
	if (!p->scope_stk) p->scope_stk = malloc(sizeof(struct symbol_table*));
	else p->scope_stk = realloc(p->scope_stk, sizeof(struct symbol_table*)*p->num_stk);
	p->scope_stk[p->num_stk-1] = symbol_table_init(top);
}

struct symbol_table *pop_scope(struct parser *p)
{
	if (!p->num_stk) return NULL;
	...
	if (!p->num_stk) {
		...
		p->scope_stk = realloc(p->scope_stk, sizeof(struct symbol_table*)*p->num_stk);
	}
	return s;
}
...
{
	struct token *t;
	if ((t=match(p, t_ident))) return value_ident(t->tok);
	if ((t=match(p, t_numeric))) return value_num(t->tok);
	if (match(p, t_lparen)) {
		struct expr *e;
		if ((e=parse_expr(p))) {
			if (!match(p, t_rparen)) syntax_error(p, "Missing right parentheses");
			return e;
		}
	}
	return NULL;
...
struct expr *parse_postfix(struct parser *p)
{
	struct token *t;
	struct expr *e;
	if (!(e=parse_primary(p))) return NULL;
	while ((t=match(p, t_inc)) || (t=match(p, t_dec)) || (t=match(p, t_lbrack))) {
		if (t->type == t_lbrack) {
			struct expr *sub;
			if (!(sub=parse_expr(p)))
				syntax_error(p, "Invalid array subscript expression");
			if (!match(p, t_rbrack))
				syntax_error(p, "Missing right bracket in subscript expression");
			e = binop_init(o_add, e, sub);
			e = unop_init(o_deref, e);
		} else {
			e = unop_init(t->type==t_inc?o_postinc:o_postdec, e);
		}
	}
...
struct expr *parse_mul_expr(struct parser *p)
{
	...
	while ((t=match(p, t_asterisk)) || (t=match(p, t_rslash))) {
		...
		if (!v2) syntax_error(p, "Unexpected token in expression");
...
struct expr *parse_rel_expr(struct parser *p)
{
	...
	if (!v) return NULL;
	struct token *t;
	while ((t=match(p, t_gt)) || (t=match(p, t_lt)) || (t=match(p, t_lte)) || (t=match(p, t_gte))) {
	...
	if ((v=parse_unary(p))) {
		if ((t=match(p, t_equal))) {
			...
			if ((rhs=parse_asn_expr(p))) {
				return binop_init(dec_oper(t->tok), v, rhs);
			}
			syntax_error(p, "Missing right hand side in assignment operation");
		}
		node_free(v);
	}
	restore(p, s);
	if ((v=parse_or_expr(p))) return v;
	return NULL;
}
...
{
	struct expr *e;
	if ((e=parse_asn_expr(p))) return e;
	return NULL;
}
...
struct stmt *parse_expr_stmt(struct parser *p)
{
	struct expr *e;
	if ((e=parse_expr(p))) {
		if (!match(p, t_semic)) syntax_error(p, "Expected ';' after expression");
		return e;
	}
	return NULL;
}
...
struct stmt *parse_block(struct parser *p)
{
	if (match(p, t_lcbrack)) {
		...
		push_scope(p);
		struct stmt *s;
		while ((s=parse_stmt(p)) || (s=parse_declaration(p))) block_addstmt(block->child, s);
		if (!match(p, t_rcbrack)) syntax_error(p, "Expected '}'");
...
struct stmt *parse_loop(struct parser *p)
{
	...
	if ((t=match(p, t_while))) {
		if (!match(p, t_lparen)) syntax_error(p, "Missing left parentheses");
		if (!(cond=parse_expr(p))) syntax_error(p, "No valid expression");
		if (!match(p, t_rparen)) syntax_error(p, "Missing right parentheses");
		if (!(body=parse_stmt(p))) syntax_error(p, "Missing statement after while expression");
	...
	if ((t=match(p, t_for))) {
		...
		if (!init)
			syntax_error(p, "No valid initializer statement in for loop");
		cond = parse_expr(p);
		if (!match(p, t_semic))
			syntax_error(p, "No valid condition statement in for loop");
		iter = parse_expr(p);
		if (!match(p, t_rparen)) syntax_error(p, "Missing right parentheses");
		if (!(body=parse_stmt(p))) syntax_error(p, "Missing statement");
...
struct stmt *parse_cond(struct parser *p)
{
	...
	if ((tif=match(p, t_if)) && match(p, t_lparen)) {
		if (!(c=parse_expr(p))) syntax_error(p, "Expected expression after '('");
		if (match(p, t_rparen)) {
			struct stmt *s, *s2;
			if ((s=parse_stmt(p))) {
				if (match(p, t_else)) {
...
int parse_dir_declarator(struct parser *p, struct declaration *decl)
{
	...
	if ((t=match(p, t_ident))) {
		if (decl->ident) syntax_error(p, "Too many identifiers in declaration");
		int len = strlen(t->tok);
		decl->ident = malloc(len+1);
		memcpy(decl->ident, t->tok, len);
		decl->ident[len] = 0;
	} else if (match(p, t_lparen) && parse_declarator(p, decl)) {
		if (!match(p, t_rparen)) syntax_error(p, "Missing right parentheses in declarator");
	} else {
		return 0;
	}
	while ((t=match(p, t_lbrack)) || (t=match(p, t_lparen))) {
		if (t->type == t_lbrack) {
			if (!match(p, t_rbrack)) syntax_error(p, "Missing right bracket in array declaration");
			decl->type = types_merge(decl->type, type_init(type_array));
			...
			while ((d=parse_param(p))) {
				...
				if (paramc==1) {
					...
					paramt = malloc(sizeof(struct type *));
				} else {
					paramv = realloc(paramv, sizeof(char*)*paramc);
					paramt = realloc(paramt, sizeof(struct type *)*paramc);
				}
			...
			if (!match(p, t_rparen)) syntax_error(p, "Missing right parentheses in function ptr declaration");
			struct type *fcn;
			decl->type = types_merge(decl->type, (fcn=type_init(type_fcn)));
...
{
	struct type *t1 = NULL;
	while (match(p, t_asterisk)) t1 = types_merge(t1, type_init(type_ptr));
...
struct declaration *parse_param(struct parser *p)
{
	...
	if (match(p, t_int)) dtype = type_int;
	if (match(p, t_char)) dtype = type_char;
	if (!dtype) return NULL; /*technically wrong, types aren't required by the grammar */
...
struct stmt *parse_declaration(struct parser *p)
{
	...
	if (match(p, t_semic)) return node_init(node_decl, decl);
	if (!match(p, t_equal)) syntax_error(p, "Expected assignment expression after declaration");
	struct expr *e = parse_asn_expr(p);
	if (!e) syntax_error(p, "Invalid initializer after declaration");
	decl->initializer = e;
	if (!match(p, t_semic)) syntax_error(p, "Expected ';' after initializer");
...
{
	struct stmt *s;
	if ((s=parse_block(p))) return s;
	if ((s=parse_expr_stmt(p))) return s;
	if ((s=parse_cond(p))) return s;
	if ((s=parse_loop(p))) return s;
	//if (s=parse_declaration(p)) return s;
	if (match(p, t_semic)) return node_init(node_empty, NULL);
