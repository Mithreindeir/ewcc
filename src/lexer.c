...
const struct token_pair token_pairs[30] = {
	{ t_int, "int", 3 },
	{ t_char, "char", 4 },
	{ t_inc, "++", 2 },
	{ t_dec, "--", 2 },
...
enum token_type lex_r(char **saveptr, int *tlen, char **ls, int *ln)
{
	...
	int len = 0;
	size_t ntoken_pairs =
	    (size_t) ((sizeof(token_pairs) / sizeof(struct token_pair)));
	/*Match against static tokens */
	for (unsigned int i = 0; i < ntoken_pairs; i++) {
		if (!strncmp
		    (str, token_pairs[i].match, token_pairs[i].len)) {
