...
void node_check(struct node *n, struct symbol_table *scope)
{
	...
	case node_binop:
		...
		if (!cmp_types
		    (BINOP(n)->lhs->inf->type, BINOP(n)->rhs->inf->type)) {
			type_error(n, BINOP(n)->lhs->inf->type,
				   BINOP(n)->rhs->inf->type);
		}
	...
	case node_decl:
		...
		if (!DECL(n)->initializer)
			break;
		if (!cmp_types
		    (get_type(scope, DECL(n)->ident),
		     DECL(n)->initializer->inf->type)) {
			type_error(n, get_type(scope, DECL(n)->ident),
				   DECL(n)->initializer->inf->type);
		}
		break;
	default:
