...
{
	v->num_siblings++;
	if (!v->siblings) v->siblings = malloc(sizeof(struct vertex));
	else v->siblings = realloc(v->siblings, sizeof(struct vertex) * v->num_siblings);
...
{
	stk->num_vertices++;
	if (!stk->vertices) stk->vertices = malloc(sizeof(struct vertex*));
	else stk->vertices = realloc(stk->vertices, sizeof(struct vertex*)*stk->num_vertices);
	stk->vertices[stk->num_vertices-1] = v;
}

struct vertex *pop(struct vstk *stk)
{
	if (!stk->num_vertices) return NULL;
	...
	if (!stk->num_vertices) {
		...
		stk->vertices = realloc(stk->vertices, stk->num_vertices*sizeof(struct vertex*));
	}
	return v;
}
...
void color_graph(struct vertex **graph, int num, int k)
{
	...
	while ((v=pop(&stk))) {
		...
		int clr = k;
		for (int i = 0; i < k; i++) used_colors[i] = 0;
		for (int i = 0; i < v->num_siblings; i++) {
			if (v->siblings[i]->color >= 0 && v->siblings[i]->color < k)
