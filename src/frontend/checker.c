...
int implicit_cast(struct node **a, struct node **b, int favor)
{
	...
	if (ra && rb) {
		if ((ra > rb && favor == NEUTRAL) || favor == LHS) {
			if (favor == LHS && rb > ra) {
				printf
				    ("Warning: Possible data loss in implicit cast from ");
				print_type(ta);
				printf(" to ");
				print_type(tb);
				printf("\n");
...
void node_check(struct node **overwrite, struct symbol_table *scope)
{
	if (!overwrite)
		return;
	struct node *n = *overwrite;
	if (!n)
		return;
	struct type *nt = NULL;
	switch (n->type) {
	case node_ident:
		if (!(nt = get_type(scope, IDENT(n)))) {
			printf
			    ("Use of undeclared or out of scope variables %s\n",
			     IDENT(n));
		}
		TYPE(n) = type_copy(nt);
		/*Implicit Conversion from array to pointer to first member of array*/
	...
	case node_binop:
		...
		int favor = NEUTRAL;
		struct type *lt = TYPE(BINOP(n)->lhs), *rt =
		    TYPE(BINOP(n)->rhs);
		...
		if (BINOP(n)->lhs && lt
		    && (lt->type == type_ptr || lt->type == type_array)) {
			v1 = BINOP(n)->lhs;
		}
		if (BINOP(n)->rhs && rt
		    && (rt->type == type_ptr || rt->type == type_array)) {
			v2 = BINOP(n)->rhs;
		}
		/*The only valid operation with two pointers is subtraction and assignment */
		if (v1 && v2
		...
		if (v1 || v2) {
			...
			if (v2 && v2->type == node_cnum) {
				...
				union value *s =
				    malloc(sizeof(union value));
				s->cnum = psize;
				struct expr *scale =
				    binop_init(o_mul,
					       node_init(node_cnum, s),
					       v2);
	...
	case node_call:
		...
		if (ct->type != type_fcn) {
			...
			node_debug(n);
			printf
			    (" is not of function or function pointer type\n");
			exit(-1);
		}
	...
	case node_decl_list:
		for (int i = 0; i < DECL_L(n)->num_decls; i++) {
			...
			node_check(&d->initializer, scope);
			if (!d->initializer)
				break;
			struct type *dtype = get_type(scope, d->ident);
			if (!cmp_types(dtype, TYPE(d->initializer))) {
				if (!explicit_cast(dtype, &d->initializer)) {
					type_error(n, dtype,
						   TYPE(d->initializer));
				}
			}
			//TYPE(n) = type_copy(dtype);
		}
		break;
	default:
		break;
